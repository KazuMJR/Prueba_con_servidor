...
<head>
    <meta charset="UTF-8">
    <title>Gestión de Cursos - MINEDUC</title>
...
<div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            ...
            <p class="text-muted mb-0">Administre los cursos registrados.</p>
        </div>
    ...
    <!-- Tarjetas de cursos -->
    @if($cursos->isEmpty())
        <div class="alert alert-warning">No hay cursos registrados aún.</div>
    @else
        <div id="cursosContainer" class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
    @foreach($cursos as $curso)
        <div class="col curso-item">
            <div class="card course-card h-100 p-1">
                <div class="card-body py-2 px-3">
                    <h6 class="course-name mb-1">{{ $curso->nombre_curso }}</h6>
                    <p class="text-muted mb-1 small"><strong>ID:</strong> {{ $curso->id_curso }}</p>
                </div>
                <div class="card-footer bg-transparent border-0 pt-1 px-3">
                    <a href="{{ route('cursos.show', $curso) }}" class="btn btn-detail btn-sm w-100 mb-2">
                        Ver Detalles
                    </a>
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('cursos.edit', $curso) }}" class="btn btn-sm btn-outline-primary px-2 py-1">
                            Editar
                        </a>
...
<script>
    document.getElementById('busqueda').addEventListener('keyup', function () {
        ...
        const tarjetas = document.querySelectorAll('.curso-item');
...
<script>
    ...
    // Redirige a la ruta sin parámetros (escuelas.index)
